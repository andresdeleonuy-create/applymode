...
(function () {
  const FIELD_CATALOG = {
    ...
    nombreCompleto: {
      ...
      // "Name" a secas es súper común ("Name*", sin "Full") pero también
      // ambiguo (Company name, Team name, Username...) — por eso NO va en
      // synonyms (que da crédito con que aparezca la palabra dentro del
      // label) sino acá, donde solo cuenta si el label/name/id es
      // EXACTAMENTE esa palabra y nada más.
      exactSynonyms: ['name'],
    },
    email: {
    ...
    cv: {
      group: 'perfil', label: 'CV / Resume', inputKind: 'file',
      autocomplete: [],
      synonyms: ['cv', 'resume', 'curriculum', 'curriculum vitae', 'hoja de vida', 'subir cv', 'upload resume', 'adjuntar cv', 'attach resume'],
    },
  };
