...
(function () {
  ...
  function friendlyErrorMessage(status) {
    if (status === 429) {
      return 'Se agotó tu cuota gratuita de Gemini por ahora (límite por minuto o por día). Esperá un rato, o revisá tu cuota en aistudio.google.com. El resto de la extensión (detección y relleno automático) sigue funcionando sin la IA.';
    }
    if (status === 503) return 'Gemini está sobrecargado en este momento. Probá de nuevo en un rato.';
    if (status === 400) return 'La API rechazó el pedido — puede ser un problema con tu API key. Revisala en Opciones.';
    if (status === 403) return 'Tu API key no tiene permiso para esto — revisala en aistudio.google.com.';
    return null;
  }
  ...
  async function callGeminiOnce({ apiKey, contents, systemInstruction, responseSchema }) {
    ...
    if (!res.ok) {
      ...
      err.friendlyMessage = friendlyErrorMessage(res.status);
      throw err;
    }
